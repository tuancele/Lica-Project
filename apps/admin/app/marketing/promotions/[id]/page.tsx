...
export default function EditPromotionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) {
    return (
        <div className="h-screen flex flex-col items-center justify-center gap-2 text-blue-600">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/marketing/promotions" className="p-2 hover:bg-gray-100 rounded-full transition">
            <ArrowLeft className="w-5 h-5 text-gray-600" />
        </Link>
        <h1 className="text-2xl font-bold text-gray-800">Chỉnh sửa chương trình</h1>
      </div>
      
      {promotion && <PromotionForm initialData={promotion} promotionId={id} />}
    </div>
  );
}
