...
echo "🚀 Đang tối ưu hóa Đơn hàng & Cập nhật hiển thị ảnh..."

# ==============================================================================
# 1. BACKEND: Tối ưu Controller (Eager Loading Product)
# ==============================================================================
echo "⚙️ Tối ưu Backend: Nạp sẵn thông tin sản phẩm trong danh sách..."

# Chúng ta cần sửa hàm index() để dùng with('items.product') thay vì chỉ with('items')
...
class OrderController extends Controller
{
    ...
    {
        // TỐI ƯU: Load luôn items.product để lấy ảnh thumbnail
...
# ==============================================================================
# 2. FRONTEND: Cập nhật Danh sách đơn hàng (Orders Page)
# ==============================================================================
echo "💻 Cập nhật Frontend List: Thay ảnh thật..."
              ...
              <div className="p-4 cursor-pointer" onClick={() => router.push(\`/orders/\${order.code}\`)}>
                {order.items.map((item) => (
                  <div key={item.id} className="flex gap-4 mb-3 last:mb-0">
                    {/* KHỐI HIỂN THỊ ẢNH ĐÃ ĐƯỢC TỐI ƯU */}
                      ...
                      {item.product?.thumbnail ? (
                        <img 
                          ...
                          onError={(e) => {
                            ...
                            e.currentTarget.parentElement?.classList.add('bg-gray-200'); // Thêm màu nền fallback
                          }} 
                        />
                      ) : (
...
# ==============================================================================
# 3. FRONTEND: Cập nhật Trang chi tiết (Detail Page)
# ==============================================================================
echo "💻 Cập nhật Frontend Detail: Thay ảnh thật..."
...
echo "✅ Hoàn tất! Ảnh sản phẩm đã được cập nhật."
