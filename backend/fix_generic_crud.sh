...
echo ">>> ĐANG VÁ LỖI BUILD (FIX HANDLE DELETE)..."
...
export default function GenericCrud({ title, endpoint, hasCode = false }: Props) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      fetchData();
    } catch (err) { alert("Không thể xóa (có thể dữ liệu đang được liên kết)."); }
  };
          ...
          <tbody className="divide-y divide-gray-100">
            {data.map((item) => (
              <tr key={item.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-4 font-mono text-blue-500 text-[12px]">
                   /{endpoint === 'skin-types' ? 'loai-da' : endpoint}/{item.slug}
                </td>
...
echo ">>> Đang Rebuild Frontend..."
cd "$ADMIN_DIR"
npm run build
pm2 restart lica-admin

echo "✅ ĐÃ FIX LỖI BUILD THÀNH CÔNG!"
