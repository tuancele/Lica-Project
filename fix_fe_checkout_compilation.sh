...
echo "========================================================"
echo "   FIX LỖI BUILD FRONTEND: CẬP NHẬT TRANG CHECKOUT"
echo "========================================================"

# 1. Cập nhật trang Checkout (checkout/page.tsx)
echo ">>> Updating apps/user/app/checkout/page.tsx..."
...
export default function CheckoutPage() {
  ...
  const [formData, setFormData] = useState({
    name: '',
    phone: '',
    address: '', // Địa chỉ cụ thể từ SmartLocationInput
    note: '',
    payment_method: 'cod'
  });
  ...
  const handleLocationChange = (data: any) => {
    ...
    // Lưu fullAddress để gửi lên server
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.address || formData.address.length < 10) {
        alert('Vui lòng nhập đầy đủ địa chỉ giao hàng (Số nhà, Phường/Xã, Quận/Huyện, Tỉnh/Thành)');
        return;
    }
    ...
    try {
      ...
      if (res) {
        ...
        // Giả sử backend trả về order code hoặc hash
        const orderCode = res.code || 'SUCCESS';
...
echo "========================================================"
echo "   ĐÃ FIX LỖI BUILD! HÃY RESTART VÀ KIỂM TRA LẠI."
echo "========================================================"
