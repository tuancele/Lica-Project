...
echo "🚀 Đang nâng cấp hệ thống Tìm kiếm địa chỉ thông minh..."

# ==============================================================================
# 1. BACKEND: Thêm API Tìm kiếm địa chỉ (LocationController)
# ==============================================================================
echo "⚙️ Cập nhật Backend API..."
...
{
    // ... (Giữ nguyên các hàm getProvinces, getDistricts, getWards cũ)
    ...
    // NEW: API Tìm kiếm thông minh
    public function search(Request $request)
    {
        $q = $request->get('q');
        if (!$q || strlen($q) < 2) return response()->json(['status' => 200, 'data' => []]);

        // Tìm kiếm trong bảng Wards (Xã) và join lên District, Province
        // Limit 20 kết quả để tối ưu
        $results = Ward::query()
            ->join('districts', 'wards.district_code', '=', 'districts.code')
            ->join('provinces', 'districts.province_code', '=', 'provinces.code')
            ->where('wards.name', 'LIKE', "%{$q}%")
            ->orWhere(function($query) use ($q) {
                // Tìm kiếm kết hợp: Ví dụ user gõ "Đại Mỗ Nam Từ Liêm"
...
# Thêm Route Search
echo "🔗 Cập nhật Route..."
# Append route search vào api.php nếu chưa có
grep -q "location/search" /var/www/lica-project/backend/Modules/IAM/routes/api.php || sed -i "/Route::get('\/wards\/{district_code}', \[LocationController::class, 'getWards'\]);/a \    Route::get('/search', [LocationController::class, 'search']);" /var/www/lica-project/backend/Modules/IAM/routes/api.php


# ==============================================================================
# 2. FRONTEND: Tạo Component Smart Location Input
# ==============================================================================
echo "💻 Tạo Component SmartLocationInput..."

mkdir -p /var/www/lica-project/apps/user/components/common

cat << 'EOF' > /var/www/lica-project/apps/user/components/common/SmartLocationInput.tsx
"use client";
...
# ==============================================================================
# 3. CẬP NHẬT TRANG SHIPPING (Sử dụng Component mới)
# ==============================================================================
echo "📝 Cập nhật trang Checkout..."
...
php /var/www/lica-project/backend/artisan route:clear
cd /var/www/lica-project/apps/user
npm run build
pm2 restart lica-user

echo "✅ Hoàn tất! Hãy thử gõ 'Đại Mỗ' vào ô địa chỉ."
